package com.habileducation.thesingers.data.domain.useCase.artistFavourite

import com.habileducation.thesingers.data.domain.model.Artist
import com.habileducation.thesingers.data.domain.repository.Repository
import kotlinx.coroutines.flow.Flow

/**
 * Created by Annas Surdyanto on 06/01/22.
 *
 */
class GetFavouriteArtistImpl(private val repository: Repository) : GetFavouriteArtist {
    override fun invoke(): Flow<List<Artist>> {
        return repository.getAllFavouriteArtists()
    }
}